...
ark (dependency resolution). Nous is the single source of truth for resolving package dependencies across system, marketplace, and Flutter packages.
...
- Do not allow non-deterministic resolution — same inputs must always produce same outputs
- Do not silently drop conflicting versions — always surface conflicts to the caller
